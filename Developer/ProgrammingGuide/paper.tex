...
Version 1.7.5 for Equalizer 0.9.1+svn\\[\medskipamount]
%  \today
}
...
\item[eq::util] The eq::util namespace provides common utility classes, which
  often the usage of OpenGL functions. Most classes in this namespace are used
  by the the Equalizer client library, but are usable independently from
  Equalizer for application development.
...
\item Only the master instance of the object generates new versions, that is,
  \textsf{eq::net::Object::com\-mit} should only be called only on the master
  instance. It is possible to commit on slave instances, but special care has to
  be taken to handle possible conflicts. \sref{sSlaveCommit} covers slave object
  commits in detail.
...
Distributed objects are created by subclassing from \textsf{eq::Object} or
