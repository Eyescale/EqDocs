...
Version 1.7.1 for Equalizer 0.9+svn\\[\medskipamount]
%  \today
}
...
\lstset{language=C++}
\section*{Equalizer 0.9 Programming and User Guide}
\today
...
\textcopyright 2007-2009 \htmladdnormallink{Eyescale Software GmbH}
...
\subsection{Subpixel Compounds}

Subpixel decomposes the rendering into different jittered frusta so 
that the recomposition anti-aliases the final rendering.
The Subpixel compound is also compatible with applications doing their own software 
anti-aliasing.
...
Subpixel compounds in Equalizer are configured using the \textsf{subpixel}
parameter, using the values \textsf{[ index size ]}. The \textsf{index} 
parameter corresponds to the current ressource's index and the \textsf{size} 
is the total number of ressources needed.

The \textsf{index} has to be smaller than the ressources \textsf{size}. 
\fig{fSubpixel} illustrates a four-way subpixel decomposition.
...
\begin{figure}[ht!]\center
  \includegraphics[width=.9\textwidth]{images/subpixel.pdf}
  {\caption{\label{fSubpixel}A four-way Subpixel compound}}
\end{figure}
...
\item[Scalability] If desired, define scalability for each of your
  ...
  used as a source. 
  \begin{description}
  \item[Decomposition] On each child compound, limit the rendering task
    of that child by setting the \textsf{viewport}, \textsf{range},
    \textsf{period} and \textsf{phase}, \textsf{pixel}, \textsf{eye} or
...
\textsf{pixel} parameter adjusts the frustum so that the source channel
renders an even subset of the parent's pixels. A \textsf{subpixel} parameter
tells the source channels to render different jittered frustum to perform
anti-aliasing. Setting one or multiple attributes causes the parent's view
to be decomposed accordingly. Attributes are cumulative, that is,
intermediate compound attributes affect and therefore decompose the
rendering of all their children.
...
\end{wrapfigure}
Examples shipped with Equalizer are implemented in their own namespaces,
e.g., \textsf{eqPly} or \textsf{eVolve}. They rely mostly on subclassing
classes from the \textsf{eq} namespace, with the occasional usage of
functionality from the \textsf{eq::net} and \textsf{eq::base} namespace.

The functionality of the Equalizer server is implemented in the
\textsf{eq::server} namespace. This namespace does not yet expose an API
intended to be used by developers.

\fig{fNamespaces} shows the namespace layering for \textsf{eqPly} and
\textsf{eqServer}.
...
%\begin{figure}[h!t]
  \subfigure[]{\label{fObjectDist1}
    \includegraphics[width=.382\textwidth]{images/umlObjectSubclass}}\hfil
  \subfigure[]{\label{fObjectDist2}
    \includegraphics[width=.382\textwidth]{images/umlObjectProxy}}\hfil
  \subfigure[]{\label{fObjectDist3}
    \includegraphics[width=.382\textwidth]{images/umlObjectMI}}%
...
\item[Frustum] The same frustum parameters as defined by
  ...
  current eye pass. If the anti-aliasing idle mode is set, the frustum
  will be jittered by a vector. This part is described in \ref{sFSAA}.
...
\textsf{frameAssemble} is overwritten for the use of Subpixel compound.
The whole process is described in \ref{sFSAA}.
...
\begin{figure}[h!t]
  \subfigure[]{\label{fMono}
    \includegraphics[height=2.5cm]{images/mono}}\hfil
  \subfigure[]{\label{fStereo}
    \includegraphics[height=2.5cm]{images/stereo}}\hfil
  \subfigure[]{\label{fTracked}
    \includegraphics[height=2.5cm]{images/tracked}}
  \subfigure[]{\label{fHMD}
    \includegraphics[height=2.5cm]{images/hmd}}%
...
\subsection{\label{sFSAA}Full-scene and Software Anti-Aliasing}

The eqPly example contains support for software anti-aliasing to show 
how to combine it with the Equalizer Subpixel compound.

\subsubsection{FSAA or Supersampling algorithm}

The anti-aliasing algorithm uses here the supersampling technique which 
computes samples within a pixel using the jitter algorithm. The pixel is split
into several sub-pixels and a sample is taken randomly within the sub-pixel. 

The sub-pixel area is a function of the current jitter step and the channel's
\textsf{subpixel} description. Jitter values for all channels should be evenly 
distributed over the total sub-pixel grid and we use therefore a precomputed
prime number table.
...
Next to that, sizes of a pixel and a sub-pixel on the near plane are computed.
A random position within the sub-pixel is set as a sample position and the 
vector values are correctly defined to finally be passed to the frustum.
...
\subsubsection{SWAA in eqPly}

This is the \textsf{FrameData} class which will give the information to the
application about being in idle mode or not. \textsf{Config} will update the 
idle mode information to the new \textsf{FrameData} version depending of the 
application state.

Since the application has a notion of idle/non-idle rendering, the 
\textsf{Channel} will be able to perform anti-aliasing when no user event is
detected and more frames are requested.
...
Then, \textsf{frameViewFinish} will get the frame buffer and accumulate 
it into an accumulation buffer. At each jitter step, the average result 
contained in the accumulation buffer will be displayed until all the steps
are done.
If the current pixel viewport is different from the one saved in 
\textsf{frameViewStart}, the accumulation buffer needs also to be resized 
and the idle anti-aliasing is reset.
...
At the end, \textsf{frameFinish} will decrement the jitter step and create a
new event to inform the \textsf{Config} of how much frames which remain to 
draw.

\subsubsection{SWAA in Equalizer}

As the software anti-aliasing decomposition should also work with the idle 
anti-aliasing in the application, the accumulation buffer is part of the 
\textsf{ObjectManager}. \textsf{frameAssemble} and \textsf{frameViewFinish} 
will then share the same accumulation buffer.

As for the application, the \textsf{eq::Channel::applyFrustum} will apply the
jitter vector if it exists. Then, if the function is not overwritten in the
application but if it still uses \textsf{subpixel} compounds, anti-aliasing 
will be correctly computed in Equalizer.

The main bit here is the assembly operation. \textsf{assembleFramesUnsorted} 
and \textsf{assembleFramesSorted} are similarly modified to handle the 
Subpixel decomposition.

For each Subpixel decomposition in input frames, all input frames are 
assembled with \textsf{subpixel} decomposition. The result is then accumulated
and displayed.

For example a 2-way subpixel compound configuration like 
\textsf{[0 2] [0 2] [1 2] [1 2]} will first assemble the both \textsf{[0 2]}
input frames and accumulate the result of the two assemblies. 
Similarly, the the both \textsf{[1 2]} frames are assembled and their result 
accumulated to the previous one.
In another hand, a 4-way subpixel compound configuration will do four 
assemblies and four accumulations.
