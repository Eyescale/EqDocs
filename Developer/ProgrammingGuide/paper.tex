...
%  \textbf{RELEASE CANDIDATE}\\[\medskipamount]
  Version 1.3.3 for Equalizer 0.5+svn\\[\medskipamount]
  \today
}
...
flow visualization application for climate research\footnote{Image
  ...
  Siegen}, the \textsf{eVolve} volume renderer\footnote{Data sets
  courtesy of General Electric, USA and AVS, USA} [bottom left] the
\textsf{eqPly} polygonal renderer in a three-sided CAVE [bottom center],
and a six-node sort-last database decomposition with parallel
direct-send recomposition\footnote{Data set courtesy of Stanford
  University Computer Graphics Laboratory} [bottom right].
...
\vfill{\center\begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    1.3.3   & May 10-16, 2008 & File Format Appendix [incomplete]\\
    \hline
  \end{tabularx}}
\thispagestyle{empty}
\clearpage
...
The currently used file format is the one-to-one representation of the
server's internal data structures. Its purpose is intermediate, that
is, it will gradually be replaced by automatic resource dedection and
configuration. However the core scalability engine will always use a
similar structure as currently exposed by the file format.

The file format represents an ascii deserialization of the
server. Streaming an \textsf{eqs::Server} to an \textsf{eqBase::Log}
ostream produces a valid configuration file. Likewise, loading a
configuration file produces an \textsf{eqs::Server}.
...
individual entities of the server section. The naming convention for
attributes is:
...
double quotes '\textsf{''}'. A character has to surrounded by single
quotes '\textsf{'}'.
...
in the config file override these defaults, and are in turn overriden by
the corresponding attributes sections of the associated entities.
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  process forked of the server process. The \% tokens are replaced by the
  server at runtime with concrete data: \%h is replaced by the hostname,
  \%c by the command to launch, including command line arguments and \%n
  by a node-unique identifier. Each command line argument are surrounded
  by launch command quotes.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  \textbf{Default} & ' [POSIX]\\
                   & '' [WIN32]\\
  \textbf{Details} & The server uses command line arguments to
  launch render client nodes correctly. This arguments might contain
  white spaces, and therefore have to be surrounded by quotes to
  identify their borders. Certain launch commands or shells use
  different conventions to separate command line arguments.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  provided by most InfiniBand protocol stacks.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  certain performance
  limitations and should only be used if really necessary.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  to allocate a stereo visual, but fall back to a mono visual if
  allocation fails.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  stereo visual. When set to AUTO, the window initialization methods try
  to allocate a double-buffered visual, but fall back to a
  single-buffered visual if allocation fails.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  statistics directly, without finishing the OpenGL commands.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  planes specified. AUTO select a visual with a reasonable quality,
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  planes specified. By default no stencil planes are requested. Note
  that for depth-compositing and pixel-compositing at least one stencil
  plane is needed.\\
  \hline
\end{tabularx}\\\vfill
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  rendering. QUAD-buffered stereo use the left and right buffers of a
...
A server or node may have multiple connection descriptions.
\clearpage
{\footnotesize\begin{lstlisting}
server
{
...
For a description of connection attributes please refer to \sref{sGlobal}.
