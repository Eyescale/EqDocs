...
%  \textbf{RELEASE CANDIDATE}\\[\medskipamount]
  Version 1.3.17 for Equalizer 0.6-RC1\\[\medskipamount]
  \today
}
...
\textcopyright 2007-2008 \htmladdnormallink{Eyescale Software GmbH}
...
\vfill{\center\begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    1.3.10  & Oct 7, 2008  & Updated to reflect 2D pixel compounds\\
    1.3.11  & Oct 13, 2008 & Updated to reflect eq::net::Object cleanup\\
    1.3.12  & Oct 16, 2008 & Added \sref{sView}\\
    1.3.13  & Oct 17, 2008 & Minor update in \sref{sMessagePump}\\
    1.3.14  & Oct 21, 2008 & Improved \sref{sConnectionDescription}\\
    1.3.15  & Oct 27, 2008 & Added \sref{sDPlex}\\
    1.3.16  & Nov 10, 2008 & Updated \sref{sThreadModel}, added back page\\
    1.3.17  & Nov 13, 2008 & Added \sref{sLoadBalancing} and \sref{sAdvDPlex}, final touches\\
    \hline
  \end{tabularx}}
\clearpage
...
\end{wrapfigure}
DB\footnote{3D model courtesy of AVS, USA.} decomposes the
rendered database so that all rendering units process a part of the
scene in parallel. This mode is also known as sort-last.
...
recommended to configure parallel direct send compositing, as described
in \sref{sDirectSend}.
...
buffer. This mode supports a variety of stereo modes, including active
...
DPlex compounds\footnote{3D model courtesy of Cyberware, Inc.} assign
full, alternating frames to individual rendering units. The resulting
images are copied to the destination channel, and Equalizer
load-balancing is used to ensure a steady framerate on the destination
window. This mode is also known as time-multiplex or AFR.
...
applications are also well suited for this mode, and should choose it
over 2D compounds.
...
\begin{wrapfigure}{r}{.382\textwidth}
  \vspace{-2ex}\includegraphics[width=.382\textwidth]{images/lb.pdf}
  {\caption{\label{fLoadBalancing}\small Static 2D compound vs 2D
      load-balancing}}
\end{wrapfigure}
...
Equalizer provides automatic, application-transparent load-balancing for
2D and DB compounds. Using an appropriate load-balancer for these
compounds will adapt the 2D split or database range each frame. The
current load-balancing algorithm is a technology preview and will
improve in future versions. 

2D load-balancing gives better framerates than static decomposition in
virtually all cases. DB load-balancing is benefitial for applications
which can not precisely predict the load for their scene data. Volume
rendering is a counter-example, where the data is uniform and a static
DB decomposition typically results in a better performance.
...
on the destination channel for optimal user experience.
...
one instance of the scene data has to be maintained within a process
since all rendering threads are guaranteed to draw the same frame.
...
so that the source channel renders an evenly subset of the parent's
pixels. Setting one or multiple attributes causes the parent's view to
be decomposed accordingly. Attributes are cumulative, that is,
...
parameter, which abstract the data transmission and can be used like a
\textsf{std::stream}.
...
The \textsf{eqPly} does not track changes since it is relatively small
in size and changes frequently. Therefore, the instance and delta data
are the same, and \textsf{pack} and \textsf{unpack} are not
implemented. The change type is \textsf{INSTANCE}. The serialization
functions exploit the fact that the frame data is layed
out in one anonymous structure in memory, and transmit all the data in
one call:
...
\begin{wrapfigure}{r}{.618\textwidth}
  ...
  {\caption{\small\label{fRender}Main Render Loop}}\vspace{-5ex}
\end{wrapfigure}
...
default, per-node thread synchronization provided by Equalizer can be
relaxed by advanced applications to gain better performance through
higher asynchronicity.
...
\item[ASYNC:] No synchronization happens between render threads, except
  for synchonizing the finish of frame \textsf{current-latency}. The
  \textsf{eqPly} and \textsf{eVolve} Equalizer examples use this
  threading model, by setting it in \textsf{Node::configInit}. This
  synchonization model provides the best performance and should be used
...
\item[LOCAL\_SYNC:] In addition to the async threas model, all local
...
process. The speed-up of DPlex relies however on the capability to
...
initialization. After an successful \textsf{OSWindow} initialization,
...
INADDR\_ANY, and is therefore reachable on all interfaces. If the
server's hostname is specified, the listening socket is bound to this
address. If any hostname given is not resolvable, or any port can not be
used, server initialization will fail. 
...
process, which binds to all provided descriptions.
...
compound
{
    ...
    loadBalancer
    {
        mode   2D | HORIZONTAL | VERTICAL | DB | DPLEX
    }
