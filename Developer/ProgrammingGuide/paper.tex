...
%  \textbf{RELEASE CANDIDATE}\\[\medskipamount]
  Version 1.3.8 for Equalizer 0.5.4+svn\\[\medskipamount]
  \today
}
...
\textsf{WindowEvent}, which holds the event data in an independent
format. The original native event and this generic \textsf{WindowEvent}
...
further processing, the \textsf{WindowEvent} is passed on to
\textsf{Window::processEvent}. This \textsf{WindowEvent} no longer contains
the native event.
