...
\date{
  ...
  \end{textblock}
      ...
      Version 1.14 for Equalizer 1.6\\[\medskipamount]
  \today
}
...
Contributions by Daniel Nachbaur, Maxim Makhinya, Jonas B\"osch, Christian
Marten, Sarah Amsellem, Patrick Bouchaud, Philippe Robert, Robert Hauck and
Lucas Peetz Dulley.

\subsection*{Copyright}

\textcopyright 2007-2013 \htmladdnormallink{Eyescale Software GmbH}
...
Equalizer can be installed by using a package or building the source
...
example. Currently we provide Ubuntu packages on
launchpad\footnote{\link{https://launchpad.net/~eilemann/+archive/equalizer}}
and MacPorts portfiles\footnote{\link{https://github.com/Eyescale/portfiles}}.

Equalizer uses Buildyard and cmake to generate platform-specific build
files. Compiling Equalizer from source consist of cloning Buildyard, the
Eyescale configurations and then building Equalizer:
...
processes receive their configuration from the server. It typically runs as a
separate entity within separate threads in the application process.
...
the rendered data\-base so that all rendering units process a part of the scene
in parallel. This mode is also known as sort-last, and is very similar to the
data decomposition approach used by HPC applications.
...
the eqServer binary and using the generated configuration, saved as \textless
session\textgreater.auto.eqc.
...
\item The default value is \textsf{local} which uses all local GPUs and network
  interfaces queried using the cgl, glx, or wgl GPU modules and the sys network
  module.
...
All suitable network interfaces are used to configure the nodes, that is, the
...
\item[--eq-config-flags \textless ethernet\textbar infiniband\textgreater]
  Limit the selection of network interfaces to one of those types.
...
different viewports. Sometimes, a single window is split across multiple
projectors, e.g., by using an external splitter such as the Matrox
TripleHead2Go. In this case configure one channel for each segment, using the
channel's \textsf{viewport} to configure its position relative to the window.
...
used by the head-tracking matrix calculated by the application.  A wall is
completely defined by the bottom-left, bottom-right and top-left coordinates
relative to the origin.  A projection is defined by the position and
...
\begin{wrapfigure}{r}{.382\textwidth}
  ...
  {\caption{\label{fLayout}Layout with four Views}}\vspace{-6ex}
\end{wrapfigure}
...
with a general introduction of the API in \sref{sAPI}, followed by the
simplified Sequel API in \sref{sSeqPly} which implements common use cases to
deliver a large subset of the canonical Equalizer API introduced in
\sref{sEqualizer}. \sref{sNetwork} introduces the separate Collage network
library used as the foundation for the distributed execution and data
synchronizing throughout Sequel and Equalizer.

\section{\label{sAPI}Programming Interface}

To modify an application for Equalizer, the programmer structures the source
code so that the OpenGL rendering can be executed in parallel, potentially using
multiple processes for cluster-based execution.
...
revision. Any changes to the public API are documented in the release notes and
the file \textsf{CHANGES.txt}.
...
\subsection{\label{sHello}Hello, World!}

\begin{wrapfigure}{r}{.382\textwidth}
  \includegraphics[width=.382\textwidth]{images/eqHello.png}
...
but less generic APIs on top of each other. It furthermore relies on a number of
...
\item[hwloc, boost, vmmlib, hwsd] External libraries providing manual and
  ...
  used only internally and are not exposed through the API.
...
\section{\label{sSeqPly}The Sequel Simple Equalizer API}
...
\section{\label{sEqualizer}The Equalizer Parallel Rendering Framework}

The core Equalizer client library exposes the full feature set and flexibility
to the application developer. New application development is encouraged to use
the Sequel library, and inquire on the \textsf{eq-dev} mailing list when
specific functionality is not available in Sequel. Only advanced and complex
applications should be implemented using the Equalizer client library directly.

\subsection{\label{sEqPly}The eqPly Polygonal Renderer}
...
\textsf{eqPly} is a parallel renderer for polygonal data in the \textsf{ply}
file format. It supports nearly all Equalizer features, and can be used to
render on large-scale displays, immersive environments with head tracking and to
render massive data sets using all scalable rendering features of Equalizer. It
is a superset of \textsf{seqPly}, introduced in \sref{sSeqPly}.
...
distribution of the model data to the render client processes. \sref{sNetwork}
...
\subsubsection{\label{sNetObject}Distributed Objects}
...
\paragraph{\label{sInitData}InitData - a Static Distributed Object}
...
{\footnotesize\lstinputlisting[firstline=62, lastline=73]
  {../../../Equalizer/examples/eqPly/initData.cpp}}

\paragraph{FrameData - a Versioned Distributed Object}
...
{\footnotesize\lstinputlisting[firstline=124, lastline=131]
  {../../../Equalizer/examples/eqPly/frameData.h}}
{\footnotesize\lstinputlisting[firstline=53, lastline=81]
  {../../../Equalizer/examples/eqPly/frameData.cpp}}


\paragraph{\label{sSceneData}Scene Data}
...
\paragraph{Initialization and Exit}
...
{\footnotesize\lstinputlisting[firstline=106, lastline=114]
  {../../../Equalizer/examples/eqPly/config.cpp}}

\paragraph{Frame Control}

The rendering frames are issued by the application main loop. The
\textsf{eqPly::Config} overrides \textsf{startFrame} to update its data, commit
a new version of the frame data object, and then requests the rendering of a new
frame using the current frame data version. This version is passed to the
rendering callbacks and will be used by the rendering threads to synchronize the
frame data to the state belonging to the current frame. This ensures that all
frame-specific data, e.g., the camera position, is used consistently to generate
the frame:

{\footnotesize\lstinputlisting[firstline=274, lastline=281]
  {../../../Equalizer/examples/eqPly/config.cpp}}

The update of the per-frame shared data consist of calculating the camera
position based on the current navigation mode, and determining the idle state
for rendering. When idle, \textsf{eqPly} performs anti-aliasing to gradually
reduce aliasing effects in the rendering. The idle state is tracked by the
application and used by the rendering callbacks to jitter the frusta, accumulate
and display the results, as described in \sref{sFSAA}:

{\footnotesize\lstinputlisting[firstline=283, lastline=315]
  {../../../Equalizer/examples/eqPly/config.cpp}}
...
\textsf{Config::finishFrame} calls \textsf{Config::handle\-Events} to perform
the event handling. The default implementation processes all pending events by
calling \textsf{Config::handleEvent} for each of them.

Since \textsf{eqPly} uses event-driven execution, the config maintains a dirty
state to know when a redraw is needed.
...
{\footnotesize\lstinputlisting[firstline=446, lastline=453]
  {../../../Equalizer/examples/eqPly/config.cpp}}


\paragraph{Model Handling}

Models in eqPly are static, and therefore the render clients only need to map
one instance of the model per node. The mapped models are shared by all pipe
render threads, which access them read-only.
...
{\footnotesize\lstinputlisting[firstline=245, lastline=272]
  {../../../Equalizer/examples/eqPly/config.cpp}}

\paragraph{Layout and View Handling}

For layout and model selection, \textsf{eqPly} maintains an active view and
canvas. The identifier of the active view is stored in the frame data, which is
used by the render client to highlight it using a different background
color. The active view can be selected by clicking into a view, or by cycling
through all views using a keyboard shortcut.
...
a different types of \textsf{eq::Config}\dots\textsf{eq::Channel}.

All dynamic data is multi-buffered in \textsf{eqPly}. During initialization, the
\textsf{eqPly::Node} relaxes the thread synchronization between the node and
pipe threads, unless the configuration file overrides this. \sref{sThreads}
provides a detailed explanation of thread synchronization modes in Equalizer.
...
since it is static, and therefore all data has been retrieved during :

{\footnotesize\lstinputlisting[firstline=227, lastline=243]
  {../../../Equalizer/examples/eqPly/config.cpp}}
...
switches. Multiple GPU threads run in parallel with each other.
...
each pipe thread, which is only the frame data in \textsf{eqPly}. The frame data is a member variable of the \textsf{eqPly::Pipe}, and is mapped
to the identifier provided by the initialization data:

{\footnotesize\lstinputlisting[firstline=45, lastline=55]
  {../../../Equalizer/examples/eqPly/pipe.cpp}}

The initialization in \textsf{eq::Pipe} does the GPU-specific initialization by
calling \textsf{configInitSystemPipe}.
which is window-system-dependent. On AGL the display ID is determined, and on
glX the display connection is opened.
...
\paragraph{\label{sCarbonThread}Carbon/AGL Thread Safety}
...
\subsubsection{\label{sEqplyWIndow}Window}
...
\paragraph{Window System Interface}
...
\paragraph{\label{sObjectManager}Object Manager}
...
\paragraph{\label{sRendering}Rendering}
...
{\footnotesize\lstinputlisting[firstline=122, lastline=142]
  {../../../Equalizer/examples/eqPly/channel.cpp}}
...
{\footnotesize\lstinputlisting[firstline=589, lastline=621]
  {../../../Equalizer/examples/eqPly/channel.cpp}}
...
kD-tree\footnote{\link{http://en.wikipedia.org/wiki/Kd-tree}}
...
{\footnotesize\lstinputlisting[firstline=249, lastline=275]
  {../../../Equalizer/examples/eqPly/channel.cpp}}
...
\subsubsection{\label{sEventHandling}Event Handling}
...
Events are received and processed by the pipe thread a window belongs to. For
AGL, Equalizer internally forwards the events from the main thread, where it is
received, to the appropriate pipe thread. This model allows window and channel
modifications which are naturally thread-safe, since they are executed from the
corresponding render thread and therefore cannot interfere with rendering
operations.

\paragraph{\label{sMessagePump}Message Pump}
...
If the application disables message pumping in Equalizer, it has to make sure
the events are processed, as it often done by external widget sets such as Qt.

\paragraph{Event Data Flow}

\begin{wrapfigure}{r}{.382\textwidth}
  \includegraphics[width=.382\textwidth]{images/eventFilter.pdf}
  {\caption{\label{fEventProcessing}Event Processing}}\vspace{-2ex}
\end{wrapfigure}
Events are received by an event handler. The event handler finds the
\textsf{eq::Sys\-tem\-Win\-dow} for the event. It then creates a generic
\textsf{Event}, which holds the event data in an independent format. The
...
\textsf{Window::processEvent} is responsible for handling the event locally and
for translating it into a generic \textsf{ConfigEvent}. Pointer events are
translated and dispatched to the channel under the mouse pointer. The window or
channel \textsf{processEvent} method perform local updates such as setting the
pixel viewport before forwarding the event to the application main loop. If the
event was processed, \textsf{processEvent} has to return \textsf{true}. If
\textsf{false} is returned to the event handler, the event will be passed to the
previously installed, window-system-specific event handling function.

After local processing, events are send using \textsf{Config::sendEvent} to the
application node. On reception, they are queued in the application thread. After
a frame has been finished, \textsf{Config::finishFrame} calls
\textsf{Config::handleEvents}. The default implementation of this method
provides non-blocking event processing, that is, it calls
\textsf{Config::handleEvent} for each queued event. By overriding
\textsf{handleEvents}, event-driven execution can easily be implemented.

\fig{fEventProcessing} shows the overall data flow of an event.

\paragraph{Default Implementation}
...
Carbon events issued for AGL windows are initially received by the node
...
\paragraph{Custom Events in eqPixelBench}
...
The rendering routine of \textsf{eqPixelBench} in \textsf{Channel::frameDraw}
loops through a number of pixel formats and types. For each of them, it measures
the time to readback and assemble a full-channel image. The format, type, size
and time is recorded in a config event, which is sent to the application. The
new custom event types are defined as user events:

{\footnotesize\lstinputlisting[firstline=38, lastline=44]
  {../../../Equalizer/examples/eqPixelBench/configEvent.h}}

The \textsf{Config::sendEvent} method provides an \textsf{eq::EventOCommand} to
which additional data can be appended. The event output command is derived from
the Collage \textsf{co::DataOStream} which provides a convenient
\textsf{std::ostream}-like interface to send data between nodes. The event is
sent when the command looses its scope, that is, immediately after the following
call in \textsf{eqPixelBench::Channel}:

{\footnotesize\lstinputlisting[firstline=477, lastline=478]
  {../../../Equalizer/examples/eqPixelBench/channel.cpp}}

Each event has a type which is used to identify it by the config processing
function. On the application end, \textsf{Config::handleEvent} receives an
\textsf{eq::EventICommand}, which provides deserialization capabilities of the
received data. The underlying \textsf{co::DataIStream} does perform endian
conversion if the endianess of the sending and receiving nodes does not
match. The event data is decoded and printed in a nicely formatted way:

{\footnotesize\lstinputlisting[firstline=57, lastline=87]
  {../../../Equalizer/examples/eqPixelBench/config.cpp}}


\subsubsection{\label{sErrorHandling}Error Handling}
...
and can be queried from the \textsf{fabric::ErrorRegistry}, which is accessible
from \textsf{fabric::Global}.
...
\subsubsection{\label{sThreads}Thread Synchronization}

Equalizer applications use multiple, potentially asynchronous execution
threads. The default execution model is designed on making the porting of
...
The application or node main thread is the primary thread of each process and
executes the \textsf{main} function. The application and render clients
initialize the local node for communications with other nodes, including the
server, using \textsf{Client::initLocal}. The client derives from
\textsf{co::LocalNode} which does provide most of the communication logic`.

During this initialization, Collage creates and manages two threads
...
during \textsf{co::Object} mapping. In some special cases the command thread
executes application code, for example when a remote node maps a static or
unbuffered object, \textsf{Object::getInstanceData} is called from the command
thread.
...
\textsf{Config::exit}. Since a layout switch on a canvas may involve different
resources, pipe threads may be started and stopped dynamically at
runtime. Before such an update, Equalizer will always finish all pending
rendering operations to ensure that all resources are idle.
...
compress and transmit the images to other nodes. Both threads are only visible
to transfer and compression plugins, not to any other application code.

\begin{figure}[h!t]\center
  \includegraphics[width=.9\textwidth]{images/threads.pdf}
  {\caption{\label{fThreads}Threads within one Node Process}}
\end{figure}

\fig{fThreads} provides an overview on all the threads used by Equalizer and
Collage. The rest of this section discusses the thread synchronization between
the main thread and the pipe threads.


\paragraph{\label{sThreadModel}Thread Synchronization Models}
...
and experimentation using test applications.

The following thread synchronization models are implemented:
\begin{description}
\item[ASYNC:] No synchronization happens between the pipe render threads, except
  for synchronizing the finish of frame \textsf{current-latency}. The
  \textsf{eqPly} and \textsf{eVolve} Equalizer examples activate this threading
  model by setting it in \textsf{Node::configInit}. This synchronization model
  provides the best performance and should be used by all applications which
  multi-buffer all dynamic, frame-specific data.
...
operations to the application node, allowing it to run asynchronous to the
rendering nodes. If the machine running the application process should also
contribute to rendering, a second node on the same host can be used to perform
off-screen draw and readback operations for the application node process.

\paragraph{\label{sAdvDPlex}DPlex Compounds}
...
\paragraph{Thread Synchronization in Detail}
...
\subsubsection{OpenGL Extension Handling}
...
multi-context support. The build system favors an existing GLEW\_MX
installation, but will fall back to a builtin implementation. Applications
should use \textsf{eq/client/gl.h} to include the proper GLEW headers. If the
preprocessor define \textsf{EQ\_IGNORE\_GLEW} is set, no GLEW headers will be
included into application code by any Equalizer header file.
...
function pointer in this table and calling the function.
...
\textsf{glewGetContext} that returns the pointer to the GLEWContext of the
appropriate window, e.g., as done by the \textsf{eqPly} kd-tree rendering
classes:
...
\subsubsection{Window System Integration}

Communicating with GPUs, creating windows and handling events is operating
system dependent in OpenGL. Equalizer abstracts the specifities behind the
\textsf{eq::Win\-dow\-Sys\-temIF} interface and provides an implemenation for
glX/X11, AGL/Carbon and WGL. This interface class defines a factory to
instantiate \textsf{SystemPipe}, \textsf{SystemWindow} and
\textsf{MessagePump}. Its constructor registers the available instances used at
runtime, that is, the class should be instantiated exactly once per process by
the implementation.

The \textsf{SystemPipe} abstracts GPU-specific handling, most notably the
implementation does query the display resolution and sets it on its
\textsf{eq::Pipe} during initialization. It also contains a
\textsf{GLXEWContext} and \textsf{WGLEWContext} dispatch table for the glX and
WGL window system, respectively.

The \textsf{SystemWindow} interface externalizes the details of the windowing
API from the \textsf{eq::Window} implementation and facilitates the integration
with new windowing APIs and custom applications. The system window implements
the core functionality for Equalizer to interface with the underlying window
system and is described in detail below.

The \textsf{MessagePump} ensures that OS events are processed and is called by
Equalizer from the main and render threads regularly. Typically it only needs to
be customized when integrating a totally new window system.

Equalizer provides sample implementations for the supported window systems in
the \textsf{agl}, \textsf{glx} and \textsf{wgl} subfolders. These sample
implementations are intended to be sub-classed and various steps in the window
initialization can be overwritten and customized.
...
format selection. \fig{fOSWindow} shows the UML class hierarchy for the system
window implementations.

\paragraph{\label{sDrawableConfig}Drawable Configuration}
...
\paragraph{AGL Window Initialization}
...
\paragraph{GLX Window Initialization}
...
{\footnotesize\lstinputlisting[firstline=71, lastline=105]
  {../../../Equalizer/eq/client/glx/window.cpp}}


\paragraph{WGL Window Initialization}
...
\subsubsection{\label{sTracking}Stereo and Immersive Rendering}
...
and asymmetric eye positions. Equalizer optionally supports tracking devices
using the VRPN or OpenCV libraries.

An Equalizer configuration contains a number of observers. Each observer
represents one tracked entity. Most immersive configurations have one observer,
but it is possible to support multiple tracked viewers in the same
configuration, e.g., to use two HMDs.

Each observer has its own head matrix, eye positions and focus information for
tracking. Typically each observer receives tracking data from a different
device. The observer may set the name of a VRPN tracking device or the index of
the OpenCV camera to be used to track this observer. The VRPN tracker is
expected to deliver tracking data in the correct coordinate system, that is,
using meter units with the same origin as the frustum descriptions in the
Equalizer configuration file. The OpenCV tracker is limited to 2.5D tracking due
to the limited amount of information gained by face detection.

\paragraph{Stereo Rendering}
...
'moved' to the position of the left and right eye, as shown in
\fig{fStereo}. The stereo frusta are asymmetric. The stereo convergence plane is
the same as the projection surface, unless specified otherwise using the focus
distance API (see \sref{sFocusDistance}).
...
\paragraph{Immersive Rendering}

In immersive visualization, the observer is tracked in and the view frusta are
adapted to the viewer's position and orientation, as shown in
\fig{fTracked}. The transformation $origin \rightarrow viewer$ is set by the
integrated tracking code or by the application using
\textsf{Observer::setHead\-Matrix}, which is used by the server to compute the
absolute eye positions, and consequently the view frusta.
...
recommended way is to emit an \textsf{OBSERVER\_MOTION} event, which has to
contain the observer identifier and a \textsf{Matrix4f} head matrix. Equalizer
optionally implements head tracking using VRPN or OpenCV, and uses this
mechanism to inject the asynchronously computed tracking matrix:

{\footnotesize\lstinputlisting[firstline=194, lastline=194]
  {../../../Equalizer/eq/client/detail/cvTracker.cpp}}

This event will be dispatched to the given observer instance in the application
process during the next \textsf{Config::handleEvents}. The observer will update
its head matrix based on the event data:

{\footnotesize\lstinputlisting[firstline=167, lastline=171]
  {../../../Equalizer/eq/client/observer.cpp}}
...
\paragraph{\label{sFocusDistance}Focus Distance}
...
%\begin{wrapfigure}{r}{.382\textwidth}
  \subfigure[]{\label{fFixed}
    \includegraphics[scale=.27]{images/focusFixed}}\hfil
  \subfigure[]{\label{fOrigin}
    \includegraphics[scale=.27]{images/focusOrigin}}\hfil
  \subfigure[]{\label{fObserver}
    \includegraphics[scale=.27]{images/focusObserver}}
...
changed at runtime for each \textsf{observer}. This allows applications to
expose the focal plane in their user interface, or to automatically calculate it
based on the scene and view direction.
...
distance parameter to dynamically change the stereo convergence.

The focus distance calculation relative to the origin allows to change the focus
independent of the observer position by separating the projection surface from
the stereo convergence plane. The convergence plane of the first wall in the
negative Z direction is moved to be at the given focus distance, as shown in
\fig{fOrigin}. All other walls are moved by the same relative amount. The
movement is made from the view of the central eye, thus leaving the mono frustum
unchanged. \fig{fOrigin} shows the new logical 'walls' used for frustum
calculations in white, while the physical projection from \fig{fFixed} is still
visible.
...
\paragraph{\label{sScaling}Application-specific Scaling}
...
the virtual environment by Equalizer. Common metric scale factors are provided
as constants.

\subsubsection{\label{sLayoutAPI}Layout API}
...
\paragraph{Subclassing and Data Distribution}
...
{\footnotesize\lstinputlisting[firstline=39, lastline=46]
  {../../../Equalizer/examples/eqPly/view.cpp}}
{\footnotesize\lstinputlisting[firstline=76, lastline=83]
  {../../../Equalizer/examples/eqPly/view.cpp}}

\paragraph{Run-time Layout Switch}
...
Initializing an entity can fail. If a failure occurs and runtime reliability is
not active, the server will exit the whole configuration, and send an
\textsf{EXIT} event to the application node. The exit event will cause the
application to exit, since it resets the config's running state.

\paragraph{Run-time Stereo Switch}
...
segment and the compound.  The view defines which stereo mode is active: mono or
...
\paragraph{Frustum Updates}

Frustum parameters can be changed at runtime for views and segments by the
application. View frusta are typically changed for non-fullscreen
...
The view and segment are derived from \textsf{eq::Frustum} (\fig{fViewUML}), and
the application process can set the wall or projection parameters at
runtime. For a description of wall and projection parameters please refer to
\sref{sFrustum}. The new data will be effective for the next frame. The frustum
of a view overrides the underlying frustum of the segments.
...
\end{wrapfigure}
When a window is resized, a \textsf{CHANNEL\_RESIZE} event is generated. If the
corresponding channel has a view, \textsf{Channel::processEvent} sends a
\textsf{VIEW\_RESIZE} event to the application. This event contains the
identifier of the view. The config event is dispatched to
\textsf{View::handleEvent} on the application thread. Using the original size
and wall or projection description of the view, a new wall or projection is
...
\subsubsection{\label{sROI}Region of Interest}
...
assembly to only the relevant pixels. This is particularly important for DB
compounds, where less screen space on each channel is covered as resources are
added to the decomposition. \fig{fROIDB} illustrates this for a two-way and
four-way decomposition. Declaring the ROI alleviates the increasing compositing
cost as resources are added to a DB compound.
...
The second optimization is important for load-balanced 2D compounds. The 2D load
equalizer uses internal timing statistics to form a 2D load grid for computing
the split of the next frame. Without ROI, an even load within each tile has to
be assumed. With ROI, an even load within the declared, smaller region of
interest can be used. This causes a smaller error in the load estimation, and
therefore a more accurate load prediction.

\fig{fROI} illustrates this load grid for a four-way 2D compound with ROI (top)
and without ROI (bottom).

\paragraph{ROI in eqPly}
...
current implementation merges all regions of a single channel into one region
for compositing and load-balancing. Later Equalizer versions may use different,
...
\subsubsection{\label{sCompositing}Image Compositing for Scalable Rendering}

Two task methods are responsible for collecting and compositing the result image
during scalable rendering. Scalable rendering is a use case of parallel
rendering, where multiple channels are contributing to a single view. This
requires reading back the pixel data from the source GPU and assembling it on
the destination GPU.
...
Please refer to the Equalizer API documentation on the website for the full
specification for compression plugins. The Lunchbox and Equalizer DSOs double as
...
\paragraph{\label{sDirectSend}Parallel Direct Send Compositing}
...
\end{wrapfigure}
In direct send compositing, each rendering channel is also responsible for the
sort-last composition of one screen-space tile. It receives the framebuffer
pixels for its tile from all the other channels. The size of one tile decreases
linearly with the number of source channels, which keeps the total amount of
pixel data per channel constant.
...
explaining and comparing this algorithm to the binary-swap algorithm.
...
\paragraph{Frame, Frame Data and Images}
...
\paragraph{\label{sCompositor}The Compositor}
...
images on the GPU, or use a CPU-based compositor and then transfer the
preintegrated result to the GPU. The high-level entry points automatically
select the best algorithm. The CPU-based compositor uses OpenMP to accelerate
its operation.
...
\paragraph{Custom Assembly in eVolve}

The \textsf{eVolve} example is a scalable volume renderer. It uses 3D
texture-based volume rendering, where the volume is intersected by view-aligned
slices. The slices are rendered back-to-front and blended to produce the final
image, as shown in \fig{fSlices}\footnote{Volume Data Set courtesy of: SFB-382
  of the German Research Council (DFG)}.
\begin{figure}[h!t]
  \subfigure[]{\label{fVolResult}
    \includegraphics[width=.45\textwidth]{images/volResult.png}
  }\hfil
  \subfigure[]{\label{fSlices}
    \includegraphics[width=.45\textwidth]{images/slices.pdf}
  }%
  {\caption{Final Result(a) of \fig{fBlendPerspective} using Volume Rendering
      based on 3D Texture Slicing(b).}}
\end{figure}
...
(less data update per time step and channel). Since the amount of texture memory
needed for each node decreases linearly, sort-last rendering makes it possible
to render data sets which are not feasible to visualize with any other approach.
...
\subsubsection{\label{sFSAA}Subpixel Processing}
...
\paragraph{Transparent Software Anti-Aliasing}
...
\textsf{eq::Channel} jitter the frustum when a subpixel parameter is set.
Furthermore, the \textsf{Compositor} uses an \textsf{eq::Accum} buffer to
accumulate and display the results of a subpixel decomposition in
\textsf{assembleFramesUnsorted} and \textsf{assembleFramesSorted}.

\paragraph{Software Anti-Aliasing in eqPly}
...
and a sample is taken randomly within each subpixel. The samples are
...
{\footnotesize\lstinputlisting[firstline=465, lastline=488]
  {../../../Equalizer/examples/eqPly/channel.cpp}}
...
rendering result after assembly. When all the steps are done the config will
...
{\footnotesize\lstinputlisting[firstline=331, lastline=350]
  {../../../Equalizer/examples/eqPly/channel.cpp}}
...
{\footnotesize\lstinputlisting[firstline=547, lastline=568]
  {../../../Equalizer/examples/eqPly/channel.cpp}}

The \textsf{FrameData} class holds the applications idle mode. The
\textsf{Config} updates the idle mode information depending on the application's
state. Each \textsf{Channel} performs anti-aliasing when no user event requires
a redraw.
...
{\footnotesize\lstinputlisting[firstline=496, lastline=502]
  {../../../Equalizer/examples/eqPly/channel.cpp}}
...
{\footnotesize\lstinputlisting[firstline=508, lastline=534]
  {../../../Equalizer/examples/eqPly/channel.cpp}}


\paragraph{Subpixel Compounds in eqPly}
...
When an application passes an \textsf{eq::Accum} object to the compositor, the
compositor adds subpixel decomposition result into this accumulation
...
\subsubsection{\label{sStatistics}Statistics}

\paragraph{Statistics Gathering}
...
visualization. Furthermore, the server also receives these events, which are
...
\paragraph{\label{sStatisticsOverlay}Statistics Overlay}
...
\subsubsection{\label{sCUDA}GPU Computing with CUDA}
...
\paragraph{CUDA Memory Management}

Equalizer does not provide any facility to perform memory transfer from and to
the CUDA devices. This is entirely left to the programmer.


\section{\label{sNetwork}The Collage Network Library}

The Collage network library provides a peer-to-peer communication
...
\item[Connection] A stream-oriented point-to-point communication
  line. Different implementations of a connection exists. The connections
...
cannot be used for communications. A connected connection can be used to read
or write data to the communication peer. A listening connection can accept
connection requests.
...
basic command dispatch is implemented in the \textsf{co::Dispatcher} class, from
which \textsf{co::Node} and \textsf{co::Object} are sub-classed.
...
used to identify the receiver, registered queue and method. The method
\textsf{dispatchCommand} pushes the packet to the registered queue and sets the
registered command function on the \textsf{ICommand}. When the commands are
dequeued by the processing thread the registered method is executed using
\textsf{ICommand::invoke}.

A command function groups the method and \textsf{this} pointer, allowing to call
a C++ method on a concrete instance. If no queue is registered for a certain
command, \textsf{dispatchCommand} directly calls the registered command function
directly.
...
connections to communicate with each other by sending \textsf{co::OCommand}s.
...
allows node and resource discovery. This requires using a Lunchbox library
supporting dns\_sd. The service "\_collage.\_tcp" is used to announce the
presence of a listening \textsf{LocalNode} using the ZeroConf protocol to the
network, unless the local node has no listening connections. The node identifier
and all listening connection descriptions are announced using keys starting with
"co\_". Internally Collage uses this information to connect unknown nodes by
using the node identifier alone.
...
\end{wrapfigure}
\fig{fNetNode} shows the communication between two nodes. When the remote node
sends a command, the listening node receives the command and dispatches it from
the receiver thread using the method \textsf{dispatchCommand}. The default
implementation knows how to dispatch commands of type node or
object. Applications can define custom data types for commands, and then have to
extend \textsf{dispatchCommand} to handle these custom data types.

Node commands are directly dispatched using
\textsf{Dispat\-cher::\-dis\-patchCommand}. For object commands, the appropriate
object is found on the local node and \textsf{Object::dis\-patchCommand} is
called on the instance to dispatch the command.

If \textsf{dispatchCommand} returns false, the command will be
re-dispatched later. This is used if an object has not been mapped
locally, and therefore the command could not be dispatched.

If an application wants to extend communication on the node level, it can define
its own datatype for commands, define custom node commands or use custom
commands.

Commands with a custom datatype use a command type greater or equal than
\textsf{COMMAND\_CUSTOM}. The receiving local node has to override
\textsf{LocalNode::dis\-patch\-Command} to handle them.

Custom node commands use the command type \textsf{COMMAND\_NODE} and any command
greater than \textsf{CMD\_NODE\_CUSTOM}. By registering a
\textsf{co::CommandFunc} on the receiving local node for these commands, the
\textsf{co::Dispatcher} dispatch and invoke mechanism is used
automatically.

These two aforementioned mechanisms are very efficient but require a clear,
linear inheritance from \textsf{co::LocalNode}. In more complex scenarios, e.g.,
when two libraries share the same local node for communication, custom commands
can be used. Custom commands are indentified by a unique 128 bit integer, which
is either generated randomly or based on a hash of a unique URI:

{\footnotesize\lstinputlisting[firstline=25, lastline=26]
  {../../../Collage/tests/customCommands.cpp}}

A specialized \textsf{Node::send} method transmits a
\textsf{CustomOCommand}. The application has to register a custom command
handler on the receiving side, which will be invoked during processing of the
command on the given thread queue:

{\footnotesize\lstinputlisting[firstline=78, lastline=84]
  {../../../Collage/tests/customCommands.cpp}}

Like any other output command, \textsf{CustomOCommand}s allow streaming
additional data to the command:

{\footnotesize\lstinputlisting[firstline=86, lastline=87]
  {../../../Collage/tests/customCommands.cpp}}
...
listening local node, which manages the network communication and command
...
One instance of an object is registered with its local node, which makes this
object the master instance. Slave instance on the same or other nodes are mapped
to this master. During mapping they are initialized by transmitting a version of
the master instance data. During commit, the change delta is pushed from the
master to all mapped slave objects, using multicast connections when
available. Slave objects can also commit data to their master instance, which in
turn may recommit it to all slaves, as described in \sref{sSlaveCommit}.
Objects can push their instance data to a set of nodes, as described in
\sref{sObjectPush}.
...
the application to \textsf{eq::Config::init}. This identifier typically contains
the identifier of a static distributed object, and is passed by Equalizer to all
...
\begin{description}
  \item[STATIC] The object is not versioned nor buffered. The instance data is
    serialized whenever a new slave instance is mapped. The serialization
    happens in the command thread on the master node, i.e., in parallel to the
    application threads. Since the object's distributed data is supposed to be
    static, this should not create any race conditions. No additional data is
    stored.
  \item[INSTANCE] The object is versioned and buffered. The instance and delta
    data are identical, that is, only instance data is serialized. The
    serialization always happens before \textsf{LocalNode::registerObject} or
  ...
  \item[DELTA] The object is versioned and buffered. The delta data is typically
    ...
    serialized before \textsf{Object::commit} returns. The delta data is
    transmitted to slave instances for synchronization. Previous instance data
    is saved to be able to map old versions.
  \item[UNBUFFERED] The object is versioned and unbuffered. No data is stored,
    and no previous versions can be mapped. The instance data is serialized from
    the command thread whenever a new slave instance is mapped, i.e., in
    parallel to application threads. The application has to ensure that this
    does not create any thread conflicts. The delta data is serialized before
...
distribution for \textsf{co::Object}. The \textsf{co::Serializable} data
...
object to be distributed during the next commit. It is easier to use, but
...
\begin{description}
\item[Inherit from \textsf{co::Serializable}:]
The base class will provide the dirty bit management and call serialize and
deserialize appropriately. By optionally overriding
\textsf{get\-Chan\-ge\-Type}, the default versioning strategy might be changed:

{\footnotesize\lstinputlisting[firstline=35, lastline=46]
  {../../../Equalizer/examples/eqPly/frameData.h}}

\item[Define new dirty bits:]
Define dirty bits for each data item by starting at
\textsf{Serializable::DIRTY\_CUSTOM}, shifting this value consecutively for each
new dirty bit:

{\footnotesize\lstinputlisting[firstline=124, lastline=131]
  {../../../Equalizer/examples/eqPly/frameData.h}}

\item[Implement serialize and deserialize:]
For each object-specific dirty bit which is set, stream the corresponding data
item to or from the provided stream. Call the parent method first in both
...
{\footnotesize\lstinputlisting[firstline=214, lastline=218]
  {../../../Equalizer/examples/eqPly/frameData.cpp}}
\end{description}
...
application has to explicitly mark the deserialized data dirty and call
\textsf{commit} to pack and push the changed data to all slaves.
...
\item Two different slave instances commit changes simultaneously which may
  result in conflicts.
\item A master instance applies data to an already changed object.
...
\item Write serialization code defensively, e.g.:
  ...
  \item When adding a new child to an object, check that it has not been added
    ...
    \item When removing a child, transmit the child's identifier instead of its
      index for removal.
    \item Modify a variable by serializing its value (instead of the operation
      modifying it), so that the last change will win.
  \end{itemize}
\end{itemize}

\subsubsection{\label{sObjectPush}Push Object Distribution}
...
initiated from an object and transmits the data to a set of given nodes. It
allows more efficient initialization of a group of slave instances when
multicast is used, since for all nodes only a single instance data is
transmitted.

The transfer is initiated using \textsf{Object::push}. On all nodes given to the
push command, \textsf{LocalNode::objectPush} is called once the data is
received. The push operation prefers multicast connections, that is,
\textsf{objectPush} is called on all nodes of the multicast groups used, even
when they are not part of the node list given to the push command.

The push command transmits two application-specific variables: a group
identifier and an object type.
...
transmitting the scene identifier to the master node. The master node collects
all requests and then traverses the given scene graph pushing all scene objects
using this scene identifier as the group identifier. The render client nodes
will filter the received object pushes against their set of pending requests.
...
The \textsf{LocalNode::objectPush} callback is called from the command thread
once the instance data is ready. It receives the aforementioned group identifier
and object type as well as the object identifier and the \textsf{DataIStream}
containing the instance data. The latter is typically used to call
\textsf{Object::applyInstanceData} on the newly created object.

The default implementation of \textsf{objectPush} calls registered push
handlers. For each group identifier, a separate handler function may be
registered using \textsf{LocalNode::registerPushHandler}.

A push operation does not establish any master-slave object mapping, that is,
the receiving side will typically use \textsf{LocalNode::mapObject} with
\textsf{VERSION\_NONE} to establish a slave mapping.

\subsubsection{Communication between Object Instances}

Objects can send commands to each other using \textsf{Ob\-ject\-OCommand}, which
is derived from \textsf{OCommand}. The object command extends the command by the
object identifier and an object instance identifier. The object output command
is created by \textsf{Object::send} and transmitted when it looses its scope.
...
session with the correct object ID. Multiple object instances per node are used
...
\end{wrapfigure}
When the command is intended for a specific instance, set the corresponding
instance identifier during \textsf{Object::send}.

Distributed objects provide data synchronization functionality, as described
above. Additionally, applications can send command commands by creating custom
object commands with a command greater than \textsf{CMD\_OBJECT\_CUSTOM}.
...
execute the rendering tasks, the server send the task command command to the
...
as well as custom command commands to implement the barrier logics. A
...
\subsection{ObjectMap}

The \textsf{co::ObjectMap} is a specialized \textsf{co::Serializable} which
provides automatic distribution and synchronization of \textsf{co::Object}s.
All changed objects are committed when \textsf{co::ObjectMap} is committed, and
sync() on slave instances updates mapped objects to their respective versions.

The objects on slave ObjectMap instances are mapped explicitly, either by
providing an already constructed instance or using implicit object creation
through the \textsf{co::ObjectFactory}. The factory needs to be supplied upon
construction of the \textsf{co::ObjectMap} and implement creation of the desired
object types. Implicitly created objects are owned by the object map which
limits their lifetime. They are released upon unmapping caused by deregister(),
explicit unmapping or destruction of the slave instance ObjectMap.

In Sequel, the \textsf{co::ObjectMap} is used to distribute and synchronize all
objects to the rendering frames. The InitData and FrameData objects are
implicitely known and managed by Sequel, and other objects might be used in
addition. The commit and synchronization of the object map, and consequently all
its objects, is automatically performed in the correct places, most importantly
the commit at the beginning of the frame, and the sync when accessing an object
from the renderer.
...
with \textsf{--eq-} and all Collage command line options with
\textsf{--co-}. The following options are recognized:
...
\item[--eq-gpufilter] applies the given regular expression against
  \textsf{nodeName:port.device} during autoconfiguration and only uses the
  matching GPUs.
...
\item[--eq-config \textless sessionName\textbar filename.eqc\textgreater] is
  ...
  \sref{sHWSDUsage}.
\item[--eq-config-flags]{\bf \textless multiprocess \textbar\ multiprocess\_db
  \textbar\ ethernet \textbar\ infiniband \textbar\ 2D\_horizontal
  \textbar\ 2D\_vertical \textbar\ 2D\_tiles\textgreater} is used as input for
  the autoconfiguration to tweak the configuration generation.
...
\item[--co-listen \textless connection\textgreater] configures the
  ...
  multiple times.
%\item[-v, -vv] Set the default logging level to \textsf{INFO} or \textsf{VERB},
%  respectively.
\end{description}

\section{\label{aFileFormat}File Format}

The current Equalizer file format is a one-to-one representation of the server's
internal data structures. Its purpose is intermediate, that is, it will
gradually be replaced by automatic resource detection and configuration. However
the core scalability engine will always use a similar structure as currently
exposed by the file format.

The file format represents an ASCII deserialization of the
server. Streaming an \textsf{eq::server::Server} to an \textsf{lunchbox::Log}
...
\begin{tabularx}{\textwidth}{|l|X|}
  ...
  \textbf{Value} & OFF \textbar \ ON\\
  \textbf{Default} & OFF\\
  \textbf{Details} & Determines if the pipe thread enables interoperability
  between CUDA and OpenGL or not. When set to ON, the CUDA device is configured
  on which the thread executes the CUDA device code. If no graphics adapter is
  specified in the config file, the one providing the highest compute
  performances on the particular node is chosen.  When set to OFF, no CUDA
...
\begin{tabularx}{\textwidth}{|l|X|}
  \hline
  \textbf{Name} & EQ\_WINDOW\_IATTR\_HINT\_GRAB\_POINTER\\
  \textbf{Value} & OFF \textbar \ ON\\
  \textbf{Default} & ON\\
  \textbf{Details} & Enables grabbing the mouse pointer outside of the window
...
verbs protocol on Linux and Windows. RSP provides reliable multicast over
UDP. PIPE uses a named pipe for fast interprocess communication on Windows.

RDMA connections provide the fastest implementation for InfiniBand fabrics. SDP
is slower than RDMA and faster than IP over InfiniBand. Note that you can also
use the transparent mode provided by most InfiniBand implementations to use SDP
with TCP connections by preloading the SDP shared library.
...
{\footnotesize\lstinputlisting[firstline=91, lastline=100]
...
{\footnotesize\lstinputlisting[firstline=102, lastline=111]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
{\footnotesize\lstinputlisting[firstline=113, lastline=131]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
The device identifies the graphics adapter. For the GLX window system this is
the screen number, i.e., the number after the dot in the DISPLAY description
(:0.\textbf{1}). The OpenGL output is restricted by glX to the GPU attached to
selected screen.
...
{\footnotesize\lstinputlisting[firstline=132, lastline=142]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
{\footnotesize\lstinputlisting[firstline=144, lastline=166]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
{\footnotesize\lstinputlisting[firstline=167, lastline=176]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}

\subsubsection{Observer Section}

An observer represents a tracked entity, i.e, one user. It has a name, eye
separation, focal plane parameters and tracking device settings. The name of an
observer can be used for debugging, it has no influence on the execution of
Equalizer. It can be used to reference the observer in views, in which case the
name should be unique. Not all views have to be tracked by an observer.
The focal plane parameters are described in \sref{sFocusDistance}. The OpenCV
camera identifies the camere device index to be used for tracking on the
application node. The VRPN tracker identifies the VRPN device tracker device
name for the given observer.

{\footnotesize\lstinputlisting[firstline=181, lastline=192]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}

\subsubsection{Layout Section}

A layout represents a set of logical views on one or more canvases. It
...
{\footnotesize\lstinputlisting[firstline=194, lastline=197]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
{\footnotesize\lstinputlisting[firstline=197, lastline=218]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
barrier of the same name are synchronized. Hardware swap synchronization is
...
{\footnotesize\lstinputlisting[firstline=221, lastline=246]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
{\footnotesize\lstinputlisting[firstline=247, lastline=269]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
...
adjusts the output image zoom to monitor another canvas. A tile equalizer
automatically sets up tile queues between the destination and all source
channels.

For a description of compound attributes please refer to \sref{sGlobal}.

A wall or projection description is used to define the view frustum of the
compound. The frustum is normally inherited from the view or segment. The
frustum is inherited and typically only defined on the topmost compound. The
last specified frustum description is used. Sizes are specified in
meters. \fig{fFrusta} illustrates the frustum parameters. Setting a frustum on a
compound is discouraged, a proper view and segment description should be used
instead. View frusta override segment frusta which override compound frusta.
...
{\footnotesize\lstinputlisting[firstline=271, lastline=358]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
{\footnotesize\lstinputlisting[firstline=362, lastline=363]
  {../../../equalizergraphics/documents/design/fileFormat.shtml}}
