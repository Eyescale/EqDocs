...
Version 1.5.10 for Equalizer 0.6+svn\\[\medskipamount]
%  \today
}
...
\lstset{language=C++}
\section*{Equalizer 0.6 Programming and User Guide}
\today
...
\listoffigures \vfill{\center\begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    1.5.10  & Jul 28, 2009 & Split in User and Programming Guide, added \sref{sNamespaces}\\
    \hline
  \end{tabularx}}
\clearpage
...
Equalizer is the next step in the evolution of middleware for scalable
OpenGL applications. Existing solutions, such as OpenGL Multipipe SDK,
Cavelib and VRJuggler, implement a subset of Equalizer. In other areas,
e.g., tracking device support, some of them provide more functionality.
...
should only be used if using a different, but synchronized rendering
...
graphics card used as a source for scalable rendering.

Sometimes display segments cover only a part of the graphics card
output. In this case it is adviced to configure a non-fullscreen window
...
Configure one \textsf{channel} for each desired rendering in each window
area. Typically one full-screen channel per window is used. Name the
channel using a unique, easily-identifiable name.
...
increase the performance. It is adviced to study and understand the
basic configuration files shipped with the Equalizer configuration,
before attempting to write compound configurations.

For configurations using canvases and layouts without scalability, the
configuration file loader will create the appropriate compounds.
...
To adapt an application for Equalizer, the programmer structures the
...
\item[eq::base] The eq::base namespace provides C++ classes to abstract
  ...
  providing a high-resolution timer. Classes in this namespace are well
  documented on the API documentation on the Equalizer website, and are
...
\item[eq] The core namespace for the Equalizer client library. The class
  hierarchy in this namespace closely models the configuration file
  format. The classes in the \textsf{eq} namespace are the main subject
...
\textsf{eq::net::Object} is subclassed.
...
identifier is used to address nodes, e.g., in order to query connection
information to connect to the node. Nodes use connections to communicate
with each other by sending packets subclassed from
