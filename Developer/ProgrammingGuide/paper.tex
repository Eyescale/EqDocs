...
{\center\begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    1.11.1  & Mar 20, 2013 & Adapt to new head tracking event\\
    \hline
  \end{tabularx}}
\clearpage
...
recommended way to integrate them into Equalizer is to emit an
\textsf{OBSERVER\_MOTION} event, which has to contain the observer's object
identifier and a \textsf{Matrix4f} head matrix. Equalizer optionally implements
camera-based head tracking using OpenCV, and uses this mechanism to inject the
asynchronously computed tracking matrix:

{\footnotesize\lstinputlisting[firstline=194, lastline=195]
  {../../../Equalizer/eq/client/cvTracker.cpp}}
...
{\footnotesize\lstinputlisting[firstline=92, lastline=97]
...
implement distributed processing independently or complementary to the
...
\item[ICommand] The other side of OCommand. Is dispatched to Object or Node to
...
Nodes and objects communicate using packets derived from \textsf{co::Packet}.
The basic packet dispatch is implemented in the \textsf{co::Dispatcher} class,
from which \textsf{co::Node} and \textsf{co::Object} is sub-classed.
