...
\title{Equalizer Programming Guide\\
  {\vspace{-1ex}\footnotesize\sffamily\mdseries
...
\date{
  ...
  Version 0.9.4 for Equalizer 0.4\\[\medskipamount]
  \today
}
...
Engineering contributions by Maxim Makhinya and Jonas B\"osch.
...
\vfill{\center\begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    \hline
    0.9.4   & Oct 26, 2007 & release candidate 1\\
...
shared-memory system with a one or multiple graphics cards up to
...
\subsection{Execution Modeal and Thread Safety}
...
be rendered. It drives the whole application, as shown in \fig{model}
...
process since all rendering threads are quaranteed to draw the same frame.
...
An \textsf{eq::Node} is the representation of a single computer in a
cluster. One operating system process of the render client will be used
...
the current implementation it is also one operating system
...
framebuffer object (FBO). The window holds
...
are not instantiated during initialization. The rendering tasks for the
...
its database, and an \textsf{eye} pass can selectively render different
stereo passes. Setting one or multiple attributes causes the parent's
view to be decomposed accordingly. Attributes are cumulative, that is,
...
detail, and relevant design decision, caveats and other remarks are
raised.
...
rendering entities introduced in \sref{sResource}, and two other classes
...
The local initialization of a node creates a local listening socket, so
that the node. Therefore the \textsf{eq::Client} can communicate over
the network with other nodes, such as the server and the rendering
clients.
...
when specified by a command-line argument. The latter is useful for
...
one (\fig{fAsync}). With \textsf{eqPly}, a speedup of 15\% has been
observed on a five-node rendering cluster when using a latency of one
instead of no
...
rendered, and is needed to provide an accurate measurement of the
framerate:
...
and deserializing its data. Serialization in \textsf{getInstanceData}
and deserialization in \textsf{applyInstanceData} is performed by
...
Versioned objects have to override \textsf{isStatic} to return
\textsf{false}, which indicates that they are versioned. The current
...
the same and set in the constructor. The default implementation will
...
\textsf{eq::Con\-fig::exit}, and then deregisters the initialization and
...
\textsf{eq::Pipe} does GPU-specific initialization, which is
window-system-dependent. On AGL the display ID is determined, and on glX
the display connection, to be used by the child windows, is opened.
...
The window exit function deallocates all OpenGL objects when the state object is
...
current. Additionally, \textsf{eq::Window::configExit} is called to
...
The OM is a template class. The template type is the key which is used to
...
\begin{wrapfigure}{r}{.382\textwidth}
  \vspace{-8ex}\includegraphics[width=.382\textwidth]{images/DB.png}
...
the pipe threads that created the windows. AGL receives the events from
application or node main thread. Whenever the term \textbf{event thread}
is used, it refers to the thread receiving the event, such as a per-node
...
\end{wrapfigure}
The main idea for direct send is to parallelize the costly recomposition
for database (sort-last) decomposition. With each additional source
channel, the amount of pixel data to be composited grows linearly. When
using the naive approach of compositing all frames on the destination
channel, this channel quickly becomes the bottleneck in the
system. Direct send distributes this workload evenly across all source
...
through the full texture. Therefore they have to be blended
...
time-dependend data (less data per time step and channel). Since the
...
in \fig{fBlendPerspective}. When the angle becomes bigger than
