...
%  \textbf{RELEASE CANDIDATE}\\[\medskipamount]
  Version 1.1.11 for Equalizer 0.4+svn\\[\medskipamount]
  \today
}
...
\vfill{\center\begin{tabularx}{\textwidth}{|l|l|X|}
    ...
    1.1.8   & Jan 15, 2008 & Detailed window initialization based on
                             latest refactoring\\
    1.1.9   & Jan 16, 2008 & Detail distributed object usage\\
    1.1.10  & Jan 28, 2008 & Add creation thread for NodeFactory entities\\
    1.1.11  & Feb 20, 2008 & Add advanced chapter for thread synchronization\\
    \hline
  \end{tabularx}}
\thispagestyle{empty}
\clearpage
...
Any parallel rendering application keeps this basic execution model and
extends it by separating the rendering code from the main event
loop. This rendering coded is then executed in parallel on different
resources. This model is followed by Equalizer, thus making application
...
graphs for example do implement multi-buffered data.
...
\subsubsection{Object Usage for Rendering}
...
this initial object is mapped by the \textsf{eq::Node}, and contains
...
Applications distributing their scene graph, rather then relying on a
shared filesystem like \textsf{eqPly}, use the frame data also as an
entry point to their scene graph data structure. \fig{fObjects} shows
one possible implementation, where the identifier and version of the
scene graph root are transported using the frame data. The scene graph
root then serializes and de-serializes his immediate children by
transferring their identifier and current version.
...
well-defined which object is mapped in which task method.
...
\textsf{Node::waitFrameStarted} to wait for the corresponding
...
method \textsf{frameDrawFinish} which is called on each resource entity
...
invoked, which is so far synchronized internally. Once all
...
This section explains window initialization in detail. It discussed in
...
\textsf{configInitAGLFullscreen} or \textsf{configInitAGLWindow}.
...
\begin{enumerate}
\item\textsf{getWGLDeviceContext} is used to get a device context which
  might be needed for window creation. Ordinary windows do not need a
  device context during creation, but PBuffers do need one. If a context
  is returned, a function pointer to delete the returned device context
  is set as well.
\item\textsf{configInitWGLDrawable} creates the drawable. The device
  context passed to \textsf{configInitWGLDrawable} is not used when
  creating a window. On success, it sets the window handle.
\item\textsf{chooseWGLPixelFormat} chooses a pixel format based on the
  window attributes. If no device context is given, it uses the device
  context from this window's window handle. The chosen pixel format is
  set on the window's device context as well as on the passed device
  context. The window needs a window handle set before this method is
  called.
\item\textsf{createWGLContext} creates an OpenGL rendering context using
  the given device context. The device context can not be 0. It does not
