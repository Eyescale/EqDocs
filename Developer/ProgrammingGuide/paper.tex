...
configuration, threading, synchronization, windowing and event handling
for portability. It is a `GLUT on steroids', providing parallel and
...
and channel task methods are executed from the pipe thread for threaded
pipes or from the main application thread for non-threaded
...
frame connects to an input frame of the same name, and transports the
...
but it is used as a test bed for most of the Equalizer features.
...
type name ambiguities, in particular for the \textsf{Window}
class. \fig{fUml} shows the most important Equalizer classes and how
they are subclassed in the \textsf{eqPly} example. Each of the classes
in the \textsf{eqPly} namespace will be discussed in this section.
...
\end{wrapfigure}
Each channel is responsible for the sort-last compositing of one 2D
tile. He receives the framebuffer pixels for his tike from all the other
channels. The size of this tile decreases linearly with the number of
source channels. 
...
algorithm:
\begin{itemize}
\item Selection of color and/or depth readback
\item Restricting the read back to a part of the rendered area
