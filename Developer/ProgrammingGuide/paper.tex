...
\date{
  ...
  Version 0.9.3 for Equalizer 0.4\\[\medskipamount]
  \today
}
...
Torvalds. CAVELib is a registered trademark of the University of
Illinois. The CAVE is a registered trademark of the Board of Trustees of
the University of Illinois at Chicago. All other trademarks and
...
% rephrase ( aber ich weiss auch nicht wie :) )
...
If you have any question regarding Equalizer programming, this programming
guide, or specific problems you encountered, please direct them to the 
 \textsf{eq-dev} mailing list\footnote{see \link{http://www.equalizergraphics.com/lists.html}}.
...
and by compiling the source code. After installing Equalizer, please
...
restructuring work required for Equalizer is needed anyway in order to
parallelize the application for rendering.
...
functions to implement certain functionality, such as the application's
...
children. The frustrum can be specified as a wall, which is completely
defined by the bottom-left, bottom-right and top-left coordinates
relative to the origin, or as a projection, which is defined by the
position and head-pitch-roll orientation of the projector, as well as
the horizontal and vertical field-of-view and distance of the projection
...
application of medium complexity. Its focus lies not on rendering features
or visual quality, but on providing a working show case for the integration
of most of the Equalizer features. 
...
After finishing, the application and \textsf{eqPly} are
...
\begin{wrapfigure}{r}{.6\textwidth}
  \vspace{-3ex}\includegraphics[width=.6\textwidth]{images/render.pdf}
  {\caption{\small\label{fRender}Main Rendering Loop}}\vspace{-3ex}
\end{wrapfigure}
